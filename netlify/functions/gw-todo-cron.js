...
// 멱등: todo:sent:<mid>.ids[항목id]=날짜 로 항목당 1회만. 사용자 문서(priv:*)는 절대 되쓰지 않는다(클라 저장과 경합 0).
...
exports.handler = async function (event) {
  ...
  for (const k of (l.keys || [])) {
    ...
    const dueNow = list.filter(function (t) { return t && t.del !== 1 && !t.done && String(t.due || '') === today && !sentDoc.ids[t.id]; });
    if (!dueNow.length) continue;
    try {
      const body = dueNow.map(function (t) { return '· ' + String(t.title || '').slice(0, 40); }).join('\n').slice(0, 200);
