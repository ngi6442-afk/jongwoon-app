...
async function runSummary(st, nowMs) {
  ...
  const tcS = await push.tierCtx();   // 스캔 1회 — bossIds와 sendTo 활성 필터 공용
  const bossIds = tcS.bossIds;
  if (!bossIds.length) {
  ...
  try {
    await push.sendTo(bossIds, { title: '결재 요청: ' + item.title, body: parts.join(' · ').slice(0, 200), url: './', tag: 'appr-' + item.id }, { ctx: tcS, primaryOnly: true });
  } catch (e) {}
