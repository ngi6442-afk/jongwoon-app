...
try {
  ...
  $htests = @(
    ...
    @('견적서(환경)','R_견적서_환경.hwpx',@('회귀테스트 하수관로 준설공사','10,000,000','포항시 맑은물사업본부','2099년 01월 01일')),
    @('견적서(건설)','R_견적서_건설.hwpx',@('회귀테스트 하수관로 준설공사','10,000,000','종운건설')),
    @('선금세트','R_선금세트.hwpx',@('회귀테스트 다목적강당 석면해체 및 제거공사','30,000,000','하도급지킴이','회귀테스트교육지원청','50.00'))
  )
  foreach($t in $htests){
...
Write-Output "== 5단계: 준공 정산 엔진(김천 실물 재현) =="
