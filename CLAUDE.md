...
이 레포는 종운환경·종운건설의 업무 앱(설치형 PWA)이다. 상세 설계는 **jongwoon-docs의 JW-11-02-001 ERP 설계·구축 가이드**를 먼저 정독할 것(문서체계 v2(2026-09-04) 재번호 — 구 JW-05-017. 레포 개편 3단계 전까지 경로는 `05_규정/JW-05-017_ERP설계구축가이드.md`). 문서 체계 일반 규칙은 JW-09-02-001 인증 취득 로드맵·AI 실행 가이드(구 JW-05-013, 현 경로 05_규정/JW-05-013)의 0장.
...
- M4 인허가·보수교육(licenses.json): **구현 완료(P4, 2026-07-08)**. `인허가` 탭(편집형). JW-08-04-001 인허가 관리표(구 JW-04-001) 13건을 시드로 넣되, **원천에 만료일이 비어 있어** 만료일·보수교육 예정일은 앱에서 직접 입력·수정(수정 모달). 만료일은 날짜/`해당없음`/미정(null) 지원. 날짜 입력 시 M2와 동일한 D-day 색상·대시보드·캘린더 알람 작동. 법정 만기일은 지어내지 않음(PM 입력).
- M7 입찰 레이더(bids): **구현(P6, 2026-07-22~23)**. 소스 10개(나라장터·수자원·국방·누리·순환자원·K-apt·LH·철도공단·한수원·포스코)+온비드(부동산, 별도 탭) — appdata Actions cron(평일 KST07시)+push 트리거 → gw-data `bids_ingest`(공유시크릿)로 Blobs 병합(새 id만 추가, 앱 status 절대 보존). 사이드바 **수주 그룹**(입찰/계약/부동산, 입찰·부동산은 관리자 전용·서버측 강제). 상태 new→검토→응찰→낙찰|유찰(패스는 검토 전), 낙찰→계약 시작 자동 연결. 카드 아코디언 상세(한빛입찰식: 입찰/금액/기관/공고서/개찰결과). 개찰결과 자동확인(results.py)·헬스 배너(health). 상세는 JW-11-02-001(구 JW-05-017) §8.
- ⚠️ 이 파일의 §아키텍처 서술 중 "GitHub Pages + Contents API 공용토큰"은 구식 — 실제 운영은 **Netlify(claude/gw-backend) + Functions(gw-auth/gw-data) + Blobs + 회원 로그인·서버 권한·감사로그**(2026-07-22 기준, sw v104). appdata JSON은 시드·백업.
...
- 예외: **관리자 전용 고정 탭**(bid/onbid/hr/fam/perm/progress)은 `MODULES`에 등록하지 않는다 — 권한관리 매트릭스에 노출되면 안 되는 탭이라 applyPerms의 `isAdmin()` 게이트 2줄 + 서버 403(ADMIN_ONLY)만으로 게이팅한다(2026-09-03 명문화).
...
- ⚠️ **UI 권한(실수 방지·역할 구분)이지 하드 보안이 아니다** — 공용 토큰이 브라우저에 있어 기술자는 우회 가능. 하드 보안은 JW-11-02-001(구 JW-05-017) §7의 1단계(백엔드)에서. 권한 모델은 그때 서버측 강제로 승격.
