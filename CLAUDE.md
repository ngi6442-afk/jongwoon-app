...
- M7 일감 레이더(bids): **구현(P6, 2026-07-22)**. 나라장터 오픈API 수집(appdata Actions cron 평일 KST07시) → gw-data `bids_ingest`(공유시크릿)로 Blobs 병합(새 id만 추가, 앱 status 절대 보존). `일감` 탭(모듈 bid, 대표/관리자 전용): 지역★ 우선·마감 임박순·신규 강조·상태(new→검토→참여|패스)·수동 등록(사기업). 홈 "새 일감" 타일. 상세는 JW-05-017 §8.
